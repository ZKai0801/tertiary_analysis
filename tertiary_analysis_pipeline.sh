...
# step2: merge multi-nucleotide polymorphism
echo "** Step2: merging starts at `date`"
python3 $merge_mnv ${input_vcf%%.vcf*}.step1_filtered.vcf ${reference} -o ${input_vcf%%.vcf*}.step2_merged.vcf


# step3: normalisation of vcf file
echo "** Step3: normalisation starts at `date`"
bcftools norm -m -both -f ${reference} ${input_vcf%%.vcf*}.step2_merged.vcf -o ${input_vcf%%.vcf*}.step3_normalised.vcf


# step4: annotate population freq, software prediction results etc.
echo "** Step4 annotation starts at `date`"
table_annovar.pl ${input_vcf%%.vcf*}.step3_normalised.vcf ${annovar_dir} \
